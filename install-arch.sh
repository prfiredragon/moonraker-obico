...
ensure_deps() {
  report_status "Installing required system packages... You may be prompted to enter password."
    ...
    PKGLIST="python3 python-pip python-virtualenv ffmpeg"
  ...
  yay -Sy --needed ${PKGLIST}
  #sudo apt-get install --yes ${PKGLIST}
  ensure_venv
  debug Running... "${OBICO_ENV}"/bin/pip3 install -q -r "${OBICO_DIR}"/requirements-arch.txt
  "${OBICO_ENV}"/bin/pip3 install -q -r "${OBICO_DIR}"/requirements-arch.txt
  echo ""
}
